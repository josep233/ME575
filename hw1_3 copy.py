#the purpose of this file is to solve the ten bar truss problem per ME575 hw1 problem 3.
#Joseph Carter
#Version 1.0
...
def g1(x):
    con1 = np.zeros([len(truss(x)[1]),1])
    for i in range(0,len(truss(x)[1])):
        if i != 8:
            if truss(x)[1][i] > 0:
                con1[i] = 25 * 10 **3 - truss(x)[1][i]
            else:
                con1[i] = 25 * 10 **3 + truss(x)[1][i]
        else:
            con1[i] = 75 * 10 **3 - truss(x)[1][i]
    return con1

def g2(x):
    con2 = np.ones([len(x),1])
...
for i in range(0,len(A0)):
    cons.append({'type':'ineq','fun':g1(A0[i])[i]})
for i in range(0,len(A0)):
    cons.append({'type':'ineq','fun':g2(A0[i])[i]})
